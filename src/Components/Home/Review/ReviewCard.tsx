...
const ReviewCard = ({item}:Props) => {
  ...
  return (
    <div className='flex flex-col items-center w-full lg:w-[80%] mx-auto bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-10 border border-gray-100 dark:border-gray-700'>
        ...
        {/* Quote Icon */}
        <div className='mt-6 text-6xl text-purple-200 dark:text-purple-900/50 font-serif leading-none'>"</div>
        ...
        <div className='mt-6 text-center'>
            <h2 className='text-xl font-bold text-gray-900 dark:text-white'>{item.username}</h2>
            <p className='text-purple-600 dark:text-purple-400 font-medium mt-1'>{item.userRole}</p>
        </div>
    </div>
  )
}
